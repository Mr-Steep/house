...
class SocketController extends Controller implements MessageComponentInterface
{
    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        if (isset($queryarray['id'])) {
            ...
            User::where('id', $user_id)->update(['connection_id' => $conn->resourceId, 'user_status' => 'Online']);
            $data['id'] = $user_id;
            $data['status'] = 'Online';
    ...
    public function onMessage(ConnectionInterface $conn, $msg)
    {
        ...
        if (isset($data->type)) {
            ...
            if ($data->type == 'request_chat_user') {
                ...
                foreach ($this->clients as $client) {
                    if ($client->request_send_message == $sender_connection_id[0]->connection_id) {
            ...
            if ($data->type == 'read_unread_message_user') {
                Chat::where('to_user_id', $data->to_user_id)
                    ->where('message_status','Not Send')
                    ->update(['message_status'=>'Send']);

                foreach ($this->clients as $client) {
                    $sender_connection_id = User::select('connection_id')->where('id', $data->to_user_id)->get();
                    if ($client->resourceId == $sender_connection_id[0]->connection_id) {
                        $send_data['type'] = 'response_read_unread_message_user';
                        $send_data['count'] = Chat::where('to_user_id',$data->to_user_id)
    ...
    public function onClose(ConnectionInterface $conn)
    {
        ...
        if (isset($queryarray['id'])) {
            $user_id = $queryarray['id'];
            User::where('id', $user_id)->update(['connection_id' => $conn->resourceId, 'user_status' => 'Offline']);
